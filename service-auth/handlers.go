...
func HandleGoogleCallback(c *gin.Context) {
    ...
    c.Redirect(http.StatusTemporaryRedirect, "http://localhost:5173/callback?token="+tokenString)
}
...
func findOrCreateUser(gu GoogleUser) (User, error) {
    ...
    resp, err := http.Post("http://localhost:8002/users/find-or-create", "application/json", strings.NewReader(string(payload)))
    if err != nil {
        return User{}, err
    }
    defer resp.Body.Close()
