    ...
    <div class="w-full max-w-sm mx-auto p-7 rounded-2xl shadow-2xl bg-gray-900 bg-opacity-90 border border-gray-800">
      ...
      <form @submit.prevent="submit" autocomplete="off" class="space-y-5">
        <div>
          ...
          <input type="number" min="10" step="1" v-model.number="amount" placeholder="Min 10€"
        ...
        <div>
          ...
          <input v-model="cardDisplay" maxlength="19" placeholder="1234 5678 9012 3456" inputmode="numeric"
            autocomplete="cc-number" @input="formatCard"
        ...
        <div class="flex gap-4">
          <div class="flex-1">
            ...
            <input v-model="exp" type="month" placeholder="MM/AA" autocomplete="cc-exp"
              class="w-full px-4 py-3 rounded-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:bg-gray-900 shadow-md font-semibold transition-all duration-200 font-mono" />
          </div>
          <div class="flex-1">
            <label class="block text-gray-300 mb-1 font-medium">CVC</label>
            <input v-model="cvc" maxlength="3" placeholder="123" autocomplete="cc-csc"
              class="w-full px-4 py-3 rounded-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:bg-gray-900 shadow-md font-semibold transition-all duration-200 font-mono" />
          </div>
        </div>
        <button type="submit"
        ...
        <div v-if="success" class="bg-emerald-700/80 text-white text-center py-2 rounded-lg font-semibold">{{ success }}
        </div>
      </form>
    </div>
  </div>
