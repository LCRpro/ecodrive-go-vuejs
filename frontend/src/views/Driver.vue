<template>
  <div
    class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-violet-900 flex flex-col items-center px-2 py-10 gap-8">

    <div class="flex flex-col items-center mb-12 mt-6">
      <span
        class="mb-3 px-4 py-1 rounded-full bg-gradient-to-r from-emerald-500 to-violet-700 text-white font-bold text-xs shadow uppercase tracking-widest">
        Interface chauffeur
      </span>
      <h2
    ...
    <DriverRequestedCourses v-if="displayedCourses.length && displayedCourses[0].status === 'requested'"
      :displayedCourses="displayedCourses" :requestedAddresses="requestedAddresses" :previewCourseId="previewCourseId"
      @preview="previewCourseId = $event" @accept="accept" />
    <DriverIdleMessage v-else-if="isIdle" />
    <DriverActiveCourse v-else-if="displayedCourses.length" :displayedCourses="displayedCourses" :driverId="driverId"
      @start="startCourse" @complete="completeCourse">
      <template #map>
        <DriverMapContainer :from="mapFrom" :to="mapTo" :follow="activeCourse && activeCourse.status === 'in_progress'"
          :zoomToCurrent="zoomOnMe" :currentPosition="myPosition" />
      </template>
