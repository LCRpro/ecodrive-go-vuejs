<template>
  <div
    class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-violet-900 flex flex-col md:flex-row items-center justify-center px-2 py-10 gap-8">
    <div
      ...
      <template v-if="activeCourse">
        ...
        <div
          ...
          <div class="flex items-center justify-between">
            ...
            <span>
              <span v-if="activeCourse.status === 'requested'"
                class="inline-block px-3 py-1 rounded-full bg-amber-600 text-white text-xs font-semibold">En
                attente</span>
              <span v-else-if="activeCourse.status === 'accepted'"
                class="inline-block px-3 py-1 rounded-full bg-emerald-600 text-white text-xs font-semibold">Prise en
                charge</span>
              <span v-else-if="activeCourse.status === 'in_progress'"
                class="inline-block px-3 py-1 rounded-full bg-blue-600 text-white text-xs font-semibold">En cours</span>
              <span v-else-if="activeCourse.status === 'completed'"
                class="inline-block px-3 py-1 rounded-full bg-violet-700 text-white text-xs font-semibold">Terminée</span>
              <span v-else-if="activeCourse.status === 'cancelled'"
                class="inline-block px-3 py-1 rounded-full bg-rose-700 text-white text-xs font-semibold">Annulée</span>
              <span v-else
        ...
        <div v-if="cancelError"
          class="mt-4 rounded-lg bg-red-700 bg-opacity-80 text-white p-3 text-center font-semibold">
          {{ cancelError }}
        </div>
      </template>
      <template v-else>
        <h2
          class="text-2xl font-extrabold bg-gradient-to-r from-emerald-400 via-violet-400 to-emerald-400 bg-clip-text text-transparent drop-shadow-lg text-center font-bold mb-6">
          Commander une course</h2>
        <div class="mb-4">
          <label class="text-gray-300 mb-1 block">Destination :</label>
          <input ref="inputRef" v-model="search" @input="searchAddr" placeholder="Ex: 6 rue de Paris..."
            autocomplete="off"
          ...
          <ul v-if="suggestions.length"
            class="bg-gray-800 border border-gray-700 rounded-lg shadow-lg max-h-60 overflow-auto absolute z-50  text-white">
            <li v-for="addr in suggestions" :key="addr.place_id" @click="pickSuggestion(addr)"
              class="cursor-pointer px-4 py-3 hover:bg-violet-700 transition">
              {{ addr.description }}
            </li>
          </ul>
        </div>
        <button @click="requestCourse" :disabled="!destination"
        ...
        <div v-if="price"
          class="mt-6 bg-gradient-to-br from-gray-800 via-gray-900 to-gray-950 rounded-xl border border-gray-700 px-6 py-5 shadow-lg text-sm text-gray-300 space-y-3">
    ...
    <div
      class="w-full max-w-4xl bg-gray-900 bg-opacity-80 border border-gray-800 rounded-2xl shadow-2xl p-3 h-full flex items-center justify-center">
      <GoogleMap :from="mapFrom" :to="mapTo" :marker="myPosition"
        :follow="activeCourse && activeCourse.status === 'in_progress'" :zoomToCurrent="zoomOnMe"
        :currentPosition="myPosition" class="w-full h-[500px] rounded-2xl" />
    </div>
  </div>
</template>
