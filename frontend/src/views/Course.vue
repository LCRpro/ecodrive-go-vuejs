  ...
  <div class="flex" style="gap:32px;">
    <div style="flex:1;max-width:350px;position:relative;">
      <template v-if="activeCourse">
        <h2>Course</h2>
        <div>
          <div><b>Départ :</b> {{ startAddress || 'Chargement...' }}</div>
          <div><b>Arrivée :</b> {{ endAddress || 'Chargement...' }}</div>
          <div>Distance : {{ activeCourse.distance_km }} km</div>
          <div>Durée estimée : {{ duration || '...' }}</div>
          <div>Prix : {{ activeCourse.amount }} €</div>
          <div>Status : <b>{{ displayStatus(activeCourse.status) }}</b></div>
        </div>
        <button v-if="canCancel" @click="cancelCourse" style="margin-top:10px;background:#e74c3c;color:white;">Annuler la course</button>
        <button @click="fetchActiveCourse" style="margin-top:10px;">Rafraîchir</button>
        <div v-if="cancelError" style="color:red;margin-top:12px;">{{ cancelError }}</div>
      </template>
      <template v-else>
        <h2>Commander une course</h2>
        <div>
          ...
          <input
            ...
            autocomplete="off"
          />
          <ul v-if="suggestions.length" style="background:#fff;border-radius:8px;box-shadow:0 2px 8px #0002;position:absolute;z-index:99;">
            <li v-for="addr in suggestions" :key="addr.place_id" @click="pickSuggestion(addr)" style="cursor:pointer;padding:6px;">
              {{ addr.description }}
            </li>
          </ul>
        </div>
        <button @click="requestCourse" :disabled="!destination" style="margin-top:10px;">Valider</button>
        <div v-if="error" style="color:red;margin-top:12px;">{{ error }}</div>
        <div v-if="price" style="margin-top:12px;">
          <b>Prix estimé : {{ price }} €</b><br />
          <span v-if="km">Distance : {{ km }} km</span><br />
          <span v-if="duration">Durée estimée : {{ duration }}</span>
        </div>
      </template>
    </div>
    <div style="flex:2;">
