<template>
        ...
        <tbody>
          <tr v-for="req in driverRequests" :key="req.ID" class="even:bg-gray-900 hover:bg-gray-800/80">
            ...
            <td class="text-center border border-gray-700 px-3 py-2 md:px-4 md:py-3">
              ...
              <button v-if="isAdmin" @click="openEditModal(req)"
  ...
  <template v-if="showEditModal">
    <div
      ...
      <div
        class="w-full max-w-sm mx-auto p-7 rounded-2xl shadow-2xl bg-gray-900 bg-opacity-95 border border-violet-800">
        ...
        <form @submit.prevent="saveEdit" autocomplete="off" class="space-y-5">
          <div>
            ...
            <input v-model="editingRequest.car"
              ...
              type="text" placeholder="Ex: Renault Clio" />
          </div>
          <div>
            <label class="block text-gray-300 mb-1 font-medium">Plaque</label>
            <input v-model="editingRequest.plate"
              class="w-full px-4 py-3 rounded-lg bg-gray-800 border border-gray-700 text-white placeholder-gray-400
                   focus:outline-none focus:ring-2 focus:ring-violet-500 focus:bg-gray-900 shadow-md font-semibold transition-all duration-200 font-mono"
              type="text" placeholder="Ex: AB-123-CD" />
          </div>
          <div>
          ...
          <div v-if="error" class="bg-red-700/80 text-white text-center py-2 rounded-lg font-semibold mt-2">{{ error }}
          </div>
          <div v-if="success" class="bg-emerald-700/80 text-white text-center py-2 rounded-lg font-semibold mt-2">{{
            success }}</div>
        </form>
      </div>
    </div>
