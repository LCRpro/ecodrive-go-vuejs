      ...
      <table class="min-w-[700px] w-full text-sm border border-gray-700 rounded-xl overflow-hidden bg-gray-900">
        ...
        <tbody>
          <tr v-for="u in users" :key="u.id" class="even:bg-gray-900 hover:bg-gray-800/80">
            <td class="text-gray-200 border border-gray-700 text-center px-3 py-2 md:px-4 md:py-3">{{ u.google_id }}
            </td>
            ...
            <td class="text-gray-200 border border-gray-700 text-center px-3 py-2 md:px-4 md:py-3">
              <span v-for="role in JSON.parse(u.roles || '[]')" :key="role" :class="roleColorClass(role)"
      ...
      <form @submit.prevent="saveEdit" autocomplete="off" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
          ...
          <div class="flex flex-wrap gap-2 mb-2">
            <span v-for="role in possibleRoles" :key="role.value" :class="[
        ...
        <div v-if="error" class="md:col-span-2 bg-red-700/80 text-white text-center py-2 rounded-lg font-semibold mt-2">
          {{ error }}</div>
      </form>
    </div>
  </div>
