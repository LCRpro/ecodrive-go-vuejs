  ...
  <div>
    <nav class="bg-gray-900 text-white px-4 py-3 flex items-center justify-between shadow-lg">
      <router-link to="/" class="flex items-center gap-3 group select-none">
        <img src="/logo-ecodrive.png" alt="Logo EcoDrive"
        ...
        <span
          class="text-3xl font-extrabold font-mono bg-gradient-to-r from-emerald-400 via-violet-400 to-emerald-300 bg-clip-text text-transparent drop-shadow group-hover:brightness-110 transition-colors">
      ...
      <div class="hidden md:flex items-center">
        ...
        <span v-if="isLoggedIn" class="separator" />
        <router-link v-if="isLoggedIn" to="/withdraw"
          class="px-3 py-2 rounded-lg font-semibold bg-violet-600 hover:bg-violet-700 transition-colors shadow mx-1">Retirer</router-link>
        <span v-if="isLoggedIn" class="separator" />
        ...
        <button v-if="isLoggedIn" @click="logout"
          class="ml-2 px-4 py-1 rounded-lg bg-violet-600 hover:bg-violet-700 transition-colors font-semibold">Déconnexion</button>
      </div>

      <button @click="isOpen = !isOpen" class="md:hidden focus:outline-none flex items-center" aria-label="Menu">
        <svg :class="isOpen ? 'rotate-90' : ''" class="w-7 h-7 transition-transform" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
    ...
    <transition name="slide-fade">
      <div v-if="isOpen" class="md:hidden bg-gray-900 text-white px-4 py-2 flex flex-col gap-2 shadow-lg">
        ...
        <router-link to="/course" v-if="isLoggedIn" class="nav-link-mobile" @click="isOpen = false">Commander une
          course</router-link>
        <router-link to="/driver" v-if="isLoggedIn && isDriver" class="nav-link-mobile" @click="isOpen = false">Courses
          à prendre</router-link>
        <router-link to="/deposit" v-if="isLoggedIn"
          class="w-full text-center py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 font-semibold transition-colors"
          @click="isOpen = false">Déposer</router-link>
        <router-link to="/withdraw" v-if="isLoggedIn"
          class="w-full text-center py-2 rounded-lg bg-violet-600 hover:bg-violet-700 font-semibold transition-colors"
          @click="isOpen = false">Retirer</router-link>
        <router-link to="/login" v-if="!isLoggedIn" class="nav-link-mobile"
          @click="isOpen = false">Connexion</router-link>
        <BalanceDisplay v-if="isLoggedIn" class="my-2" />
        <button v-if="isLoggedIn" @click="logout"
          class="w-full text-left px-2 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 font-semibold transition-colors">Déconnexion</button>
      </div>
    </transition>
    ...
    <footer
      class="w-full bg-gray-900 text-gray-400 pt-6 pb-8 px-6 flex flex-col md:flex-row items-center justify-between border-t border-gray-800 gap-3">
      ...
      <div class="flex flex-col md:flex-row gap-2 md:gap-6 text-sm">
        ...
        <router-link to="/conditions" class="hover:text-violet-400 underline transition">Conditions générales de
          vente</router-link>
      </div>
      <button v-if="isLoggedIn" @click="becomeAdmin" class="text-sm underline hover:text-violet-400 transition">Devenir
        admin (pour la démo)</button>
    </footer>
  </div>
</template>
